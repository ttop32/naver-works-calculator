...
근무 통계 페이지의 `csv 다운로드` 버튼을 누르면 `work_times_YYYY-MM.csv` 가 저장됩니다. 날짜별 근로시간(분)과 기준 대비 차이, 그리고 오늘까지만 집계한 컬럼이 따로 들어 있어 화면에 표시되는 **전체** / **오늘까지** 값과 맞아떨어집니다. 맨 아래에 합계 행이 분 단위와 시간 단위로 각각 붙고, 엑셀에서 바로 열리도록 UTF-8 BOM 이 들어갑니다.
